"""Tests for backend/streaming.py — binary frame encoding, frame store, CDP lifecycle."""
...
def _make_jpeg(width: int = 64, height: int = 64, color=(100, 150, 200)) -> bytes:
...
class TestBinaryFrameEncoding:
    ...
    def test_agent_id_truncated_at_32(self, sample_jpeg_bytes):
        frame = encode_binary_frame("a" * 50, sample_jpeg_bytes)
        assert frame[1:33] == b"a" * 32
...
class TestFrameStore:
    ...
    def test_get_frame_returns_jpeg_bytes(self):
        frame_store["agent-1"] = FrameData(jpeg=b"JPEG", ts=1.0)
        assert get_frame_for_delivery("agent-1") == b"JPEG"

    def test_get_all_agent_ids(self):
        frame_store["a"] = FrameData(jpeg=b"", ts=1.0)
        frame_store["b"] = FrameData(jpeg=b"", ts=1.0)
        assert set(get_all_agent_ids()) == {"a", "b"}

    def test_get_all_agent_ids_empty(self):
        assert get_all_agent_ids() == []

    def test_latest_frame_wins(self):
        frame_store["agent-1"] = FrameData(jpeg=b"OLD", ts=1.0)
        frame_store["agent-1"] = FrameData(jpeg=b"NEW", ts=2.0)
        assert get_frame_for_delivery("agent-1") == b"NEW"
...
class TestStopScreencast:
    ...
    async def test_removes_cdp_session_from_registry(self):
        streaming_mod._cdp_sessions["agent-1"] = AsyncMock()
        await stop_screencast("agent-1")
        assert "agent-1" not in streaming_mod._cdp_sessions

    @pytest.mark.anyio
    async def test_cdp_send_exception_is_swallowed(self):
        """CDP errors during teardown must not propagate (browser may be gone)."""
        mock_cdp = AsyncMock()
...
class TestStartScreencast:
    ...
    @pytest.mark.anyio
    async def test_screencast_uses_grid_settings(self, mock_page, mock_cdp):
        """CDP must be told to resize to 320×240 — no server-side PIL processing."""
        await start_screencast("agent-1", mock_page)
        params = mock_cdp.send.call_args_list[0][0][1]
        assert params["quality"] == settings.screenshot_grid_quality
        assert params["maxWidth"] == settings.screenshot_grid_width
        assert params["maxHeight"] == settings.screenshot_grid_height
    ...
    @pytest.mark.anyio
    async def test_frame_handler_stores_raw_bytes(self, mock_page, mock_cdp):
        """Frame bytes are stored verbatim — no JPEG validation in the streaming layer."""
        captured: dict = {}
        mock_cdp.on.side_effect = lambda event, fn: captured.update({event: fn})
