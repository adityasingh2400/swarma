...
"""
    ...
    def listing_task(self, item: ItemCard, package: ListingPackage) -> tuple[str, list[dict]]:
        ...
        task = f"""
You are on eBay's sell page. Follow these steps IN ORDER. Work quickly — do not repeat steps.
...
{images_str}
   Wait for ALL photos to finish uploading before continuing.

6. If you see 'Item Specifics' fields (Brand, Size, Color, Department, etc.),
   fill in what you can infer from the item name. Skip any you cannot determine.

7. For Price: The Buy It Now price field may have an auto-suggested value.
   IMPORTANT: To change the price, you MUST:
   a. Click on the price input field
   b. Triple-click or use Ctrl+A / Cmd+A to select ALL existing text
   c. Then type the new price: {price}
   If the price field still won't change after 2 attempts, accept whatever price is shown
   and move on — do NOT spend more than 2 steps on this.

8. For Shipping: Look for a shipping option. Select 'Calculated shipping' or
   'Buyer pays' if available. If no option is visible, skip this step.

9. For Description: paste this text exactly:
{package.description}

10. Click 'List it' to publish. Wait for the confirmation page.
    If you are redirected to a sign-in page, return 'AUTH_EXPIRED' immediately.
    Otherwise return the listing URL from the confirmation page.
